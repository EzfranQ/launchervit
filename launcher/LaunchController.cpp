...
void LaunchController::decideAccount()
{
    ...
    if (!m_accountToUse && accounts->count() == 0) {
        // Tell the user they need at least one account (Microsoft or offline) in order to play.
        auto reply = CustomMessageBox::selectable(m_parentWidget, tr("No Accounts"),
                                                  tr("In order to play Minecraft, you must have at least one account added. "
                                                     "You can add a Microsoft account or a local (offline) account. "
...
LaunchDecision LaunchController::decideLaunchMode()
{
    ...
    // Offline accounts always launch the full game in offline mode, no entitlement check required.
